...
public class Fluids {
	...
	public static void postInit() {
		...
		// Register new blocks
		flowingWater = new BlockFFWater(Block.waterMoving.blockID,
				Material.water, Block.waterStill.blockID,
				Block.waterMoving.blockID, "water",Block.waterMoving);
		stillWater = new BlockFFWater(Block.waterStill.blockID, Material.water,
		...
		// Register new blocks
		flowingLava = new BlockFFLava(Block.lavaMoving.blockID, Material.lava,
				Block.lavaStill.blockID, Block.lavaMoving.blockID, "lava",Block.lavaMoving);
		stillLava = new BlockFFLava(Block.lavaStill.blockID, Material.lava,
		...
		/*if (FysiksFun.settings.flowingHydrochloricAcid) {
			patchModLiquid("Still Hydrochloric Acid",
					"Flowing Hydrochloric Acid", true, 1, true, true);
		}*/
	...
	private static void patchModLiquid(String stillName, String flowingName,
			boolean hasTwoNames, int updateRate, boolean causesErosion,
			boolean leaksThrough) {

		Block oldBlockFlowing = null;
		Block oldBlockStill = null;
		for (Block b : Block.blocksList) {
			if (b != null && b.getUnlocalizedName() != null
					&& b.getUnlocalizedName().matches("tile." + flowingName))
				oldBlockFlowing = b;
			if (b != null && b.getUnlocalizedName() != null
					&& b.getUnlocalizedName().matches("tile." + stillName))
				oldBlockStill = b;
		}
		if (oldBlockFlowing == null) {
			FysiksFun.logger
					.log(Level.WARNING, "Cannot patch behaviour of block:"
							+ "tile." + flowingName
							+ " since it was not found (is the mod installed?)");
		}
		...
		if (oldBlockFlowing != null && oldBlockStill != null
				&& FysiksFun.settings.flowingLiquidOil) {
			
			Block.blocksList[oldBlockFlowing.blockID] = null;
			BlockFFFluid blockFlowing = new BlockFFFluid(oldBlockFlowing,
					oldBlockFlowing.blockID, oldBlockFlowing.blockMaterial,
					oldBlockStill.blockID, oldBlockFlowing.blockID, flowingName);
			...
			if (hasTwoNames) {
				...
				BlockFFFluid blockStill = new BlockFFFluid(oldBlockStill,
						oldBlockStill.blockID, oldBlockStill.blockMaterial,
						oldBlockStill.blockID, oldBlockFlowing.blockID, stillName);
	 ...
	 * given cell. Returns the amount of incoming liquid should be LEFT after
	 * the interaction
	 */
	public static int liquidInteract(World w, int x, int y, int z,
			int incomingBlockID, int incommingAmount, int targetBlockID,
			int targetAmount) {
		...
		int nReactions = Math.min(lavaAmount, waterAmount) / reactionStepSize
				+ 1;
		...
		/*
		 * Check if the interaction happens in air/water/gas, if so move the
		 * result as far down as possible
		 */
		int targetY = y;
		...
		for (; targetY > 0; targetY--) {
			...
			if (Fluids.stillWater.isSameLiquid(id))
				continue;
			if (Gases.isGas[id])
				continue;
			break;
			...
			for (int dy = -1; dy <= 1; dy++)
				for (int dz = -1; dz <= 1; dz++) {
					if (targetY + dy > 0 && targetY + dy < 255) {
						Chunk chunk1 = ChunkCache.getChunk(w, (x + dx) >> 4,
								(z + dz) >> 4, false);
						if (chunk1 != null
								&& chunk1.getBlockID((x + dx) & 15, targetY
										+ dy, (z + dz) & 15) == Block.obsidian.blockID)
							hasObsidianNeighbour = true;
					}
				}
		if (FysiksFun.rand.nextInt(10) < nReactions)
			w.playSoundEffect(x + 0.5, y + 0.5, z + 0.5, "random.fizz", 1.0F,
					1.0F);

		for (int i = 0; i < nReactions; i++) {
			int r = w.rand.nextInt(200);
			int newId = 0;
			if (r == 0 || (r <= 3 && hasObsidianNeighbour))
				newId = Block.obsidian.blockID;
			else if (r <= 10)
				newId = Block.stone.blockID;
			else if (r <= 20)
				newId = Block.cobblestone.blockID;

			if (newId != 0) {
				FysiksFun.setBlockWithMetadataAndPriority(w, x, targetY, z,
						newId, 0, 0);
				generated = true;
				break;
			}
		...
		for (int dist = 1; dist < 2 && steamAmount > 0; dist++) {
			for (int dir0 = 4; dir0 < 6 + 4 && steamAmount > 0; dir0++) {
				...
				if (id == 0 || id == Gases.steam.blockID) {
					int origAmount = id == 0 ? 0 : Gases.steam.getBlockContent(
							w, x1, y1, z1);
		...
		if (Block.blocksList[incomingBlockID].blockMaterial == Material.lava) {
			if (!generated)
				flowingWater.setBlockContent(w, x, y, z, waterAmount);
			return lavaAmount;
		} else {
			if (!generated)
				flowingLava.setBlockContent(w, x, y, z, lavaAmount);
			return waterAmount;
		}
	}
