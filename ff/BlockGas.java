...
public class BlockGas extends Block {
  ...
  /**
   * Schedules an update for all neighbours that are of the same gas type as we are - but only if they have strictly
   * MORE liquid than what is given
   */
  private void notifySameGasNeighboursWithMore(World w, int x, int y, int z, int limit, int delay) {
    int id;
    /*
     * id = w.getBlockId(x + 1, y, z); if ((id == blockID) && getBlockContent(w, x + 1, y, z) > limit) {
     * FysiksFun.scheduleBlockTick(w, this, x + 1, y, z, delay); } id = w.getBlockId(x - 1, y, z); if ((id == blockID)
     * && getBlockContent(w, x - 1, y, z) > limit) { FysiksFun.scheduleBlockTick(w, this, x - 1, y, z, delay); } id =
     * w.getBlockId(x, y + 1, z); if ((id == blockID) && getBlockContent(w, x, y + 1, z) > limit) {
     * FysiksFun.scheduleBlockTick(w, this, x, y + 1, z, delay); } id = w.getBlockId(x, y - 1, z); if ((id == blockID)
     * && getBlockContent(w, x, y - 1, z) > limit) { FysiksFun.scheduleBlockTick(w, this, x, y - 1, z, delay); } id =
     * w.getBlockId(x, y, z + 1); if ((id == blockID) && getBlockContent(w, x, y, z + 1) > limit) {
     * FysiksFun.scheduleBlockTick(w, this, x, y, z + 1, delay); } id = w.getBlockId(x, y, z - 1); if ((id == blockID)
     * && getBlockContent(w, x, y, z - 1) > limit) { FysiksFun.scheduleBlockTick(w, this, x, y, z - 1, delay); }
     */
  }
  ...
  /*
   * @Override public void onBlockAdded(World w, int x, int y, int z) { FysiksFun.scheduleBlockTick(w, this, x, y, z,
   * updateRate); }
   */
  /*
   * @Override public void onNeighborBlockChange(World w, int x, int y, int z, int ignore) {
   * FysiksFun.scheduleBlockTick(w, this, x, y, z, updateRate); }
   */
  @Override
