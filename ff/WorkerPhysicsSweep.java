...
public class WorkerPhysicsSweep implements Runnable {
  ...
  private static void setupDefaultPhysicsRules() {
    ...
    blockStrength[Block.cobblestone.blockID] = 80; // 10 times
  ...
  private void doSolidBlockPhysics(Chunk c, ChunkTempData tempData, ExtendedBlockStorage[] blockStorage, int x, int y, int z, int x0, int y0, int z0, int dx,
    ...
    // Propagate forces
    // TODO: introduce multiple levels of breaking, that moves gradually more and more power. Need more bits!
    ...
    if (doBreak == false && doFall == false) {
      ...
      // Active breaking but not beeing able to move is in process, we need to equalize the pressure by raising the curBreak counter
      if(debugMe) {
  ...
  private boolean fallOrBreak(Chunk c, int x0, int y0, int z0, int id, int weight, int timeNow, boolean doBreak,
    ...
    if (!blockDoPhysics[idBelow]) {
      ...
      if (!canMoveSideways) {
        curBreak = 0; // Returned through a class variable, signifies we have landed. 
        return false;
      }
