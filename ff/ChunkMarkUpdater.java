...
/** Maps a chunk into all blocks that are marked to be updated within that chunk. TODO garbage collect old instances (especially for the world!) */
class ChunkMarkUpdater {
  static int scheduledMarkUpdates=0;
  static int ticksLeft=0;
  
  public CoordinateWXZ coordinate;   
  public HashSet<CoordinateWXYZ> markList;
  ...
  public static void scheduleBlockMark(World w, int x, int y, int z) {
    ...
    if(coord == null) { coord = new CoordinateWXYZ(w,x,y,z); }
  ...
  public static void printStatistics() {
    System.out.println("Mark queue: "+ChunkMarkUpdater.markChunkQueue.size()+" Hashtable:"+markChunkHashtable.size()+" free pool:"+ markChunkFreePool.size());
  ...
  public static void doTick() {
    ...
    final double markRadiusSq_fine = 8.0d * 8.0d; // Measured in block coordinates
    ...
    while (iterator.hasNext()) {   
      ...
      if (doSend) {
        ...
        // Allow a percentage of ticks to be spent on the global update of chunks further away
  ...
  private void releaseChunk() {
    if(markList.size() != 0)
      System.out.println("ERROR - a ChunkBlockMarkList is released with a non-empty mark list, this is a BUG!");
