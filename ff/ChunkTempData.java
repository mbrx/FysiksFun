...
/** Class responsible for storing extra temporary data for chunks that contain any FF blocks. This data will become reset on server restart, so it should only be used for temporary data such as propagation of pressures or computations of paths */
public class ChunkTempData {
	...
	/* TODO - implement a mechanism for retiering old chunkTempData that have not been used for some time... */
	...
	public static ChunkTempData getChunk(World w,int x,int y,int z) {
  	...
  	if(chunk == null) {
  		//System.out.println("(getChunk) Creating new ChunkTempData: "+Util.xyzString(x>>4,  y>>4,  z>>4));
  		chunk = new ChunkTempData(w,x>>4,y>>4,z>>4);  		
  	} 
  	// else System.out.println("(getChunk) Found old ChunkTempData: "+Util.xyzString(x>>4,  y>>4,  z>>4));
  	return chunk;
  }
  ...
  public static int getTempData(World w,int x,int y,int z) {
  	...
  	ChunkTempData chunk = chunks.get(tempCoordinate);
  	if(chunk == null) {
  		//System.out.println("(getTempData) Creating new ChunkTempData: "+Util.xyzString(x>>4,  y>>4,  z>>4));
  		chunk = new ChunkTempData(w,x>>4,y>>4,z>>4);
  		return 0;
  	} else {
  		//System.out.println("(getTempData) Found old ChunkTempData: "+Util.xyzString(x>>4,  y>>4,  z>>4));
  		int baseAddr=((x&15)<<1) + ((z&15)<<5) + ((y&15)<<9);
  ...
  public static void setTempData(World w,int x,int y,int z,int data) {
  	...
  	if(chunk == null) {
  		chunk = new ChunkTempData(w,x>>4,y>>4,z>>4);
  		//System.out.println("(setTempData) Creating new ChunkTempData: "+Util.xyzString(x>>4,  y>>4,  z>>4));
  	} 
  	// else System.out.println("(setTempData) Found old ChunkTempData: "+Util.xyzString(x>>4,  y>>4,  z>>4));
  	int baseAddr=((x&15)<<1) + ((z&15)<<5) + ((y&15)<<9);
