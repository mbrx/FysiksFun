    ...
    for (int tries = 0; tries < FysiksFun.settings.plantsThirst; tries++) {
      ...
      for (int y = 0; y < 256; y++) {
        ...
        if (b == Block.plantRed || b == Block.plantYellow || b == Block.tallGrass) {
          ...
          int drinkRange = 4;
          int spreadFailures = 8; // Probabilities to NOT spread when drinking  
          int spreadRange = 12; // Probabilities to NOT spread when drinking
          int plantDensity = 4; // How many plants of this type is allowed in the same 5x5 area (roughly)
          ...
          if (!checkForWater(w,x,y,z,drinkRange,yMin,yMax,32, shouldConsume)) {
            /*
      ...
      for (int dy2 = yMin; dy2 <= yMax; dy2++) {
        ...
        if (id2 == Fluids.stillWater.blockID || id2 == Fluids.flowingWater.blockID) {
          if (consumeWater) {
            ...
            int amount = Fluids.flowingWater.getBlockContent(w, x3, y2 + dy2, z3) - 1;
      ...
      for (int y = 2; y < 192; y++) {
        ...
        if (b instanceof BlockCrops) {
          ...
          for (int dir = 0; dir < 4; dir++) {
            ...
            if (id2 == Fluids.stillWater.blockID || id2 == Fluids.flowingWater.blockID) {
              int amount = Fluids.flowingWater.getBlockContent(w, x2, y - 1, z2) - 1;
              Fluids.flowingWater.setBlockContent(w, x + dx + dx2, y - 1, z + dz + dz2, amount);
              Counters.cropsDrink++;
              foundWater = true;
              break;
            }
          ...
          if (!foundWater) {
            ...
            // if(meta > 0) b.dropBlockAsItem(w, x + dx, y, z + dz, meta - 1, 0);
          }
          continue;
        }
      }
