...
public class WorldSupportGenerator implements IWorldGenerator {
	...
	public void generate(Random random, int chunkX, int chunkZ, World world,
		...
		case 0:
			...
			break;
		default:
			// This is most likely Mythcraft or other such worlds. Assume that they are "normal" 
			generatePillarSupports(world, c, chunkX << 4, chunkZ << 4);
			break;
		}
	}
		...
		for (int tries = 0; tries < 4; tries++) {
			...
			for (y = 20; y < maxY; y++) {
				...
				if (Block.blocksList[id] == null) {
					FysiksFun.logger.log(Level.SEVERE, "Bad block id=" + id
							+ " found @" + Util.xyzString(x + dx, y, z + dz)
							+ " while generating world supports");
					break;
				}
				if (Block.blocksList[id] instanceof BlockOre)
					continue;
				break;
			}
			if (y == 20)
				continue; // This is a flatworld, don't do any pillars
			...
			for (y = 8; y < 128; y++) {
				...
				if (id == 0 || Fluids.isLiquid[id]) {
					...
					if (c.getBlockID(dx, y + tmp, dz) == 0)
						world.setBlock(x + dx, y + tmp, z + dz,
