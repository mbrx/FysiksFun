...
public class BlockPyroclastic extends BlockGas {
  ...
  public void expensiveTick(World w, Chunk c, ChunkTempData tempData0, int x, int y, int z, Random rand) {
    ...
    if((content==1 && FysiksFun.rand.nextInt(40) == 0)) {
      Fluids.flowingLava.setBlockContent(w, c, tempData0, x,y,z, BlockFluid.pressurePerContent*content/4,"[pyroclasticToLava]",null);     
    }
  }
  ...
  protected void condenseFromAltitude(World w, int x, int y, int z, Chunk origChunk, int newContent) {
    ...
    Fluids.stillLava.setBlockContent(w, x, y, z, newContent * BlockFluid.maximumContent / 16);
    return;
  }   
}
