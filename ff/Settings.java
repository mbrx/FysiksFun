...
public class Settings {
  ...
  public void loadFromConfig(Configuration config) {
    ...
    config.addCustomCategoryComment(categoryPhysics, "Everything regarding the physics of solid blocks");
    ...
    maxUpdatesPerTick = config.get(categoryGeneric, "max-client-updates-per-tick", "300",
    ...
    treeThirst = config.get(categoryEcology, "tree-thirst", "10", "Amount of water consumed by trees when enable-tree-consumption is true", Property.Type.INTEGER).getInt(10);
    treesFall = config.get(categoryEcology, "trees-fall", "true", "Trees fall in a semi-natural way when chopped down, in addition to the normal physics", Property.Type.BOOLEAN).getBoolean(true);
    ...
    plantsThirst = config.get(categoryEcology, "plants-thirst", "10", "Speed with which plants and crops drink water and plants spread themselves. (default 10)",
        Property.Type.INTEGER).getInt(10);
    weatherSpeed = config.get(categoryEcology, "weather-speed", "10", "Multiplier (> 1) for how often weather changes", Property.Type.INTEGER)
        .getInt(10);
    clayToDirtChance = config.get(categoryEcology, "clay-creation", "0", "Multiplier (normal rate 10) for how quickly underwater dirt can be converted to clay. Use zero to disable.", Property.Type.INTEGER)
        .getInt(0); 
    ...
    volcanoFrequency = config.get(categoryVolcanoes, "volcano-freqency", "100", "Frequency of volcanoes in percent (default 100)", Property.Type.INTEGER)
        .getInt(100);
    volcanoRadius = config.get(categoryVolcanoes, "volcano-radius", "5", "Maximum radius of the plume feeding a volcano (default 5, safe max 8, absolute max 16)", Property.Type.INTEGER)
        .getInt(5);
